...
statements xs ys = liftA2 mS xs ys ++ liftA2 mNS xs ys
  where
    mS x y = x ++ " is " ++ y
    mNS x y = x ++ " is not " ++ y
...
calculator op numStr = liftA2 ($) (lookup op operations) (readMaybe numStr)
  where
...
twoPersons name1 age1 employed1 name2 age2 employed2 = 
  liftA2 combine (Person <$> name1 <*> age1 <*> employed1) (Person <$> name2 <*> age2 <*> employed2)
  where
    combine p1 p2 = [p1, p2]
------------------------------------------------------------------------------
