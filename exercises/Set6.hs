...
instance Eq Name where
  Name a == Name b = if map toLower a == map toLower b then True
                     else False
...
instance Eq a => Eq (List a) where
  Empty == Empty = True
  LNode a xs == LNode b ys = if a == b && xs == ys then True
                             else False
  _ == _ = False
...
instance Eq RationalNumber where
  (RationalNumber a b) == (RationalNumber c d) = if a * d == b * c then True
                                                 else False
...
instance Num RationalNumber where
  ...
  signum (RationalNumber a b) = if a < 0 then RationalNumber (-1) 1
                                else if a > 0 then RationalNumber 1 1
                                else RationalNumber 0 1
