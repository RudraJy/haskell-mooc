...
treeMax Empty = 0
treeMax (Node a left right) = max(max a (treeMax left)) (max a (treeMax right))
...
allValues condition (Node a left right) = if condition a then allValues condition left && allValues condition right 
                                          else False
...
cull val (Node a left right) = if a == val then Empty
...
isOrdered (Node a Empty (Node b l r)) = a < b && isOrdered (Node b l r)
isOrdered (Node a (Node b l r) Empty) = b < a && isOrdered (Node b l r)
isOrdered (Node a (Node b l r) (Node c ll rr)) = b < a && a < c && isOrdered (Node b l r) && isOrdered (Node c ll rr)
...
walk [] (Node a left right) = Just a
walk (StepL:steps) (Node a left right) = walk steps left
walk (StepR:steps) (Node a left right) = walk steps right
...
search val (Node a left right) = if val == a then Just []
