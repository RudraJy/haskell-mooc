...
shout text = 
    if T.null text
        then T.empty
        else T.unwords $ zipWith ($) (cycle [T.toUpper, id]) (T.words text)
...
longestRepeat c = 
    if T.null c
        then 0
        else maximum $ map T.length $ T.group c
...
takeStrict n text = do 
    if n == 0
        then T.empty
        else TL.toStrict $ TL.take n text
...
byteRange byte = 
    if B.null byte
        then 0
        else B.maximum byte - B.minimum byte
