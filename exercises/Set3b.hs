...
sums' n i count = if count == i then [n, n + count] 
                  else n : sums' (n + count) i (count+1)
...
indexDefault (x:xs) i def = if i == 0 then x 
...
sumsOf' :: Int -> [Int] -> [Int]
sumsOf' _ [] = []
sumsOf' acc (x:xs) = let sumValue = acc + x
                     in sumValue : sumsOf' sumValue xs
