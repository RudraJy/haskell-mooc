...
longest' (a:b:[]) = if length a == length b then 
                    if head a < head b then a 
                        else b 
                    else a
...
incrementKey mainKey values = map (\(key, val)-> if key == mainKey then (key, val+1) 
                                                 else (key, val)) values
...
transfer from to amount bank = if Map.notMember from bank || Map.notMember to bank then bank
                               else if amount < 0 then bank
                               else if Map.findWithDefault 0 from bank < amount then bank
...
maxIndex arr = fst $ maximumByValue (assocs arr)
  where
    maximumByValue []           = error "Empty array"
    maximumByValue ((i, x):xs) = foldl (\acc@(iMax, xMax) (i', x') -> if x' > xMax then (i', x') else acc) (i, x) xs
