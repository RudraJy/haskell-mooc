...
isSorted s = if length s < 2 then property True else property (and $ zipWith (<=) s (tail s))
...
inputInOutput input output = 
  if and [elem x (map fst output) | x <- input] then property True else property False
...
outputInInput input output = 
  if and [length (filter (==x) input) == n | (x,n) <- output] then property True else property False
