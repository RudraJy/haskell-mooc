...
paintSolid color (Shape f) (Picture p) = Picture g where
                                                   g coord | f coord = color
                                                           | otherwise = p coord
------------------------------------------------------------------------------
...
paint (Picture p1) (Shape f) (Picture p2) = Picture g where
                                                      g coord = if f coord then p1 coord 
                                                                else p2 coord   
------------------------------------------------------------------------------
...
instance Transform Fill where
  apply (Fill color) _ = Picture p where 
                                   p _ = color
...
instance Transform Flip where
  apply (FlipX) (Picture f) = Picture g where
                                            g (Coord x y) = f (Coord (-x) y)
  apply (FlipY) (Picture f) = Picture g where
                                            g (Coord x y) = f (Coord x (-y))
  apply (FlipXY) (Picture f) = Picture g where
...
instance Transform Blur where
  apply Blur (Picture f) = Picture g where
                                        g (Coord x y) = Color (div (getRed (f (Coord x y)) + getRed (f (Coord (x+1) y)) + getRed (f (Coord (x-1) y)) + getRed (f (Coord x (y+1))) + getRed (f (Coord x (y-1)))) 5) 
                                                              (div (getGreen (f (Coord x y)) + getGreen (f (Coord (x+1) y)) + getGreen (f (Coord (x-1) y)) + getGreen (f (Coord x (y+1))) + getGreen (f (Coord x (y-1)))) 5) 
                                                              (div (getBlue (f (Coord x y)) + getBlue (f (Coord (x+1) y)) + getBlue (f (Coord (x-1) y)) + getBlue (f (Coord x (y+1))) + getBlue (f (Coord x (y-1)))) 5)
------------------------------------------------------------------------------
