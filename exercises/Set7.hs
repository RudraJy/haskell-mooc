...
member a (Set []) = False
member a (Set (x:xs)) = if a == x || member a (Set xs) then True
                        else False

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add a (Set xs) = if member a (Set xs) then Set xs
                 else Set (sort (a:xs))
...
data State = Start | Error | Finished | AddedEggs | AddedFlour | AddedSugar | AddedFlourEggs | Mixed 
  deriving (Eq,Show)

step :: State -> Event -> State
step Start AddEggs = AddedEggs
step AddedEggs AddFlour = AddedFlour
step AddedFlour AddSugar = AddedFlourEggs
step AddedEggs AddSugar = AddedSugar
step AddedSugar AddFlour = AddedFlourEggs
step AddedFlourEggs Mix = Mixed
...
passwordAllowed s (MinimumLength n) = if length s >= n then True
                                      else False
passwordAllowed s (ContainsSome cs) = if any (`elem` cs) s then True
                                      else False
passwordAllowed s (DoesNotContain cs) = if any (`elem` cs) s then False
                                        else True
passwordAllowed s (And r1 r2) = if passwordAllowed s r1 && passwordAllowed s r2 then True
                                else False
passwordAllowed s (Or r1 r2) = if passwordAllowed s r1 || passwordAllowed s r2 then True
                               else False
